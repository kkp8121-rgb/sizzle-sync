...
const bytes = fs.readFileSync(path.join(root, 'dist', 'sizzle-sync-web.zip'));
...
assert.deepEqual(files.map(file => file.name).sort(), ['.nojekyll', 'assets/chef.webp', 'assets/favicon.svg', 'assets/kitchen.webp', 'game.js', 'index.html', 'style.css'].sort());
