...
function createServer() {
  return http.createServer((req, res) => {
    let relative;
    try { relative = decodeURIComponent(new URL(req.url, 'http://localhost').pathname).replace(/^\/sizzle-sync(?=\/|$)/, '').replace(/^\/+/, ''); }
...
if (require.main === module) createServer().listen(Number(process.env.PORT) || 4175, '127.0.0.1', () => console.log('SIZZLE SYNC local server ready.'));
